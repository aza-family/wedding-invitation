<template>
  <v-responsive id="comment" class="custom-content-container">
    ...
    <div class="text-type-black text-type-16">
      <span class="text-type-olive">{{ $t("view.comment.text_1") }}</span
      >{{ $t("view.comment.text_2") }}
    </div>
    <div class="text-type-black text-type-16">
      {{ $t("view.comment.text_3") }}
    </div>
    <div
      id="lv-container"
      data-id="city"
      data-uid="MTAyMC81MzYyMy8zMDA5Ng=="
    ></div>
  </v-responsive>
</template>
...
export default {
  name: "Comments",
  async mounted() {
    const commentScript = document.querySelector(
      `script[src="${COMMENT_SCRIPT_URL}"]`
    );
